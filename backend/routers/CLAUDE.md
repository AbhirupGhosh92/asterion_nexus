...
| `agents.py` | `GET /api/agents` — the specialist roster for the homepage gallery. Auth only (no quota): listing costs no model call. Returns agents above the caller's tier too, flagged `locked`. |
