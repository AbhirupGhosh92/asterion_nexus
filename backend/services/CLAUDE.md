...
| `registry_service.py` | The admin-managed model catalog (Firestore `models`). Resolves a model id + tier to a guardrail-wrapped instance, cached per model. Hides Dify agents when the engine is unreachable. |
